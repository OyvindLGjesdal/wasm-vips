...
export CXXFLAGS="$CFLAGS"
export LDFLAGS="-L$TARGET/lib -O3 -flto WASM_OBJECT_FILES=0"
...
test -f "$TARGET/lib/pkgconfig/vips.pc" || (
  ...
  emconfigure ./autogen.sh --host=$CHOST --prefix=$TARGET --enable-static --disable-shared --disable-dependency-tracking \
    ...
    --without-openslide --without-matio --without-cfitsio --without-pangoft2 --without-imagequant --without-doxygen --without-nsgif
